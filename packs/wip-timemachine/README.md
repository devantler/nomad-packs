...
```
nomad-pack run timemachine --var message="Hola Mundo!"
```
...
balance the Consul service `timemachine-service`, which is the default value
...
- `consul_service_tags` (list of string) - The consul service name for the
  timemachine application
- `consul_service_name` (string) - The consul service name for the timemachine
  application
